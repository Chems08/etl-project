...
| 3 | **Streaming** | Cotations live (Finnhub) → topic **Kafka** → consommateur → sink PostgreSQL | `streaming/producer.py`, `streaming/consumer.py` |
...
`Python` · `yfinance` · `PostgreSQL` · `Apache Kafka` (KRaft) · `Apache Airflow` · `SQL` ·
...
- Ajouter un consommateur **Flink/Spark** à la place du consommateur Python.
